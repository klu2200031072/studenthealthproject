...
@WebServlet("/AdminSendOtpServlet")
public class AdminSendOtpServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            response.sendRedirect("adminverify_otp.jsp"); // Redirect to OTP verification page
        } catch (MessagingException e) {
            e.printStackTrace();
            request.setAttribute("errorMessage", "Error sending OTP. Please try again."); // Set error message
            request.getRequestDispatcher("adminforgotpassword.jsp").forward(request, response); // Forward to the forgot password page with error
        }
    }
}
