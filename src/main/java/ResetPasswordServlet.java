...
public class ResetPasswordServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (rowsUpdated > 0) {
                response.sendRedirect("login.jsp?reset=success"); // Redirect to login page with success message
            } else {
                response.sendRedirect("reset_password.jsp?error=1"); // Redirect back with error
            }
